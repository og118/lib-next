...
def verify_local_environment():
    if host_name != 'localhost' and host_name != '127.0.0.1' and host_name != 'postgres':
        ...
        if answer.lower() == 'yes':
            print(f"You have chosen to proceed with environment using host: {host_name}")
            time.sleep(1)
        else:
            print(f"You have chosen not to proceed with environment using host: {host_name}")
            sys.exit(0)
    elif db_name != 'testdb':
        answer: str = input(f"Database: {db_name} seems to be other than the testdb. "
                            f"Are you sure you want to proceed? Type - Yes to continue or No to abort")
        if answer.lower() == 'yes':
            print(f"You have chosen to proceed with environment using database: {db_name}")
            time.sleep(1)
        else:
            print(f"You have chosen not to proceed with environment using database: {db_name}")
            sys.exit(0)
    else:
        print(f"Environment using host: {host_name} and database: {db_name} seems to be local testing.")
